...
//		}
	...
	public function getAddress($lat, $lng) {
		...
		if ($use_curl) {
			...
			$data = json_decode($str, true);
			if (isset($data["results"]) && is_array($data["results"])) {
				$street = $data["results"][1]["formatted_address"];
				$ac = $data["results"][1]["address_components"];
				$len = count($ac);
				$i = 0;
				$city = "";
				while ($i<$len) {
					if (in_array("locality,political", $ac[$i]["types"])) $city = $ac[$i]["long_name"];
					else if (in_array("country", $ac[$i]["types"])) $city .= ",".$ac[$i]["short_name"];
					$i++;
				}
				$meta = array(
			...
			$data = json_decode($str, true);
			$status = $data["status"];
			if (isset($data["results"]) && is_array($data["results"]) && $status != "ZERO_RESULTS") {
				$street = $data["results"][0]["formatted_address"];
				$ac = $data["results"][0]["address_components"];
				$len = count($ac);
				$i = 0;
				$city = "";
				while ($i<$len) {
					if (in_array("administrative_area_level_2", $ac[$i]["types"])) $city = $ac[$i]["long_name"];
					else if (in_array("administrative_area_level_3", $ac[$i]["types"])) $city .= ",".$ac[$i]["short_name"];
					$i++;
				}
				$meta = array(
					"status"=>200,
